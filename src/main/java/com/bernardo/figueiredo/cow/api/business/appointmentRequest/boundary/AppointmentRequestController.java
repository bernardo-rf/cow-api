...
public class AppointmentRequestController {
    ...
    public ResponseEntity<AppointmentRequestDTO> getAppointmentRequest(@PathVariable long appointmentRequestId)
        ...
        try {
            AppointmentRequest appointmentRequest =
                    appointmentRequestRepository.getAppointmentRequest(appointmentRequestId);
            if (appointmentRequest != null) {
                AppointmentRequestDTO appointmentRequestDTO =
                        appointmentRequestService.convertToDTO(appointmentRequest);
    ...
    public ResponseEntity<List<AppointmentRequestFullInfoDTO>> getAppointmentRequestByIDBovine(
            @PathVariable long bovineId) throws Exception {
        try {
            List<AppointmentRequest> appointmentRequestList =
                    appointmentRequestRepository.getAllBovineAppointmentRequest(bovineId);
    ...
    public ResponseEntity<List<AppointmentRequestFullInfoDTO>> getAppointmentRequestByIDUser(@PathVariable long userId)
        ...
        try {
            List<AppointmentRequest> appointmentRequestList =
                    appointmentRequestRepository.getAllUserAppointmentRequest(userId);
            if (!appointmentRequestList.isEmpty()) {
                appointmentRequestDTOList = appointmentRequestService.getAppointmentsRequest(appointmentRequestList);
            }
            return ResponseEntity.ok(appointmentRequestDTOList);
    ...
    public ResponseEntity<List<AppointmentRequestFullInfoDTO>> getAppointmentRequestByIDUserRequest(
            @PathVariable long userRequestId) throws Exception {
        try {
            List<AppointmentRequest> appointmentRequestList =
                    appointmentRequestRepository.getAllUserRequestAppointmentRequest(userRequestId);
            if (!appointmentRequestList.isEmpty()) {
                appointmentRequestDTOList = appointmentRequestService.getAppointmentsRequest(appointmentRequestList);
            }
            return ResponseEntity.ok(appointmentRequestDTOList);
    ...
    public ResponseEntity<List<AppointmentRequestDTO>> createAppointmentRequest(
            @RequestBody AppointmentRequestCreateDTO appointmentRequestCreateDTO) throws Exception {
        try {
            List<AppointmentRequestDTO> appointmentRequestDTOList =
                    appointmentRequestService.createAppointmentRequest(appointmentRequestCreateDTO);
    ...
    public ResponseEntity<AppointmentRequestDTO> updateAppointmentRequest(
            @PathVariable long appointmentRequestId, @RequestBody AppointmentRequestDTO appointmentRequestDTO)
            throws Exception {
        try {
            if (appointmentRequestId == appointmentRequestDTO.getIdAppointmentRequest()) {
                AppointmentRequestDTO updatedAppointmentRequestDTO =
                        appointmentRequestService.updateAppointmentRequest(appointmentRequestDTO);
    ...
    public ResponseEntity<AppointmentRequestDTO> updateAppointmentRequest(
            @PathVariable long appointmentRequestId, @RequestParam int status) throws Exception {
        try {
            AppointmentRequestDTO updatedAppointmentRequestDTO =
                    appointmentRequestService.updateAppointmentRequestStatus(appointmentRequestId, status);
            if (updatedAppointmentRequestDTO.getIdAppointmentRequest() != 0) {
                return ResponseEntity.ok(updatedAppointmentRequestDTO);
            }
            return ResponseEntity.status(404).build();
