...
public interface BovineRepository extends JpaRepository<Bovine, Long> {
    ...
    @Query(
            value =
                    "SELECT B.* FROM cow_bovine as B WHERE B.id_user = :idOwner and B.id_bovine NOT IN (SELECT A.id_bovine FROM cow_auction as A WHERE A.auction_status != 2) and B.active = 1 ORDER BY B.serial_number ASC",
            nativeQuery = true)
    ...
    @Query(
            "SELECT b FROM Bovine b WHERE b.serialNumber = :serialNumber and b.user.idWallet = :idWallet and b.active = true")
