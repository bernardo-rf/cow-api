...
public interface AppointmentRequestRepository extends JpaRepository<AppointmentRequest, Long> {
    @Query(
            "SELECT a FROM AppointmentRequest a WHERE a.user.idUser = :idUser ORDER BY a.appointmentRequestStatus ASC, a.appointmentRequestDate ASC")
    List<AppointmentRequest> getAllUserAppointmentRequest(long idUser);

    @Query(
            "SELECT a FROM AppointmentRequest a WHERE a.bovine.idBovine = :idBovine ORDER BY a.appointmentRequestStatus ASC, a.appointmentRequestDate ASC")
    List<AppointmentRequest> getAllBovineAppointmentRequest(long idBovine);

    @Query(
            "SELECT a FROM AppointmentRequest a WHERE a.userRequest.idUser = :idUserRequest ORDER BY a.appointmentRequestStatus ASC, a.appointmentRequestDate ASC")
    ...
    @Query(
            "SELECT a FROM AppointmentRequest a WHERE a.bovine.idBovine = :idBovine and a.appointmentRequestDate = :appointmentDate")
